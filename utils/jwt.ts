...
export const getUserRoles = (user: any): Role[] => {
  const roles: Role[] = [];
  if (user.userOrganizations?.some((uo: any) => uo.role === Role.USER))
    roles.push(Role.USER);
  if (user.userRoles?.some((ur: any) => ur.role === Role.SUPER_ADMIN))
    roles.push(Role.SUPER_ADMIN);
  if (user.userRoles?.some((ur: any) => ur.role === Role.CAMPUS_ADMIN))
    roles.push(Role.CAMPUS_ADMIN);
...
export const getUserRole = (user: any): Role =>
  getHighestRole(getUserRoles(user));
